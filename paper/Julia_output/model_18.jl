# Script generated on 2025-04-12 09:39:18.024212 by sdbuildR. Please cite ***
...
function ode_func!(dSdt, S, pars, t)
	...
	# Flow to bathtub
	faucet = desired_amount .- bathtub
	
	# Flow from bathtub
	leakage = .1 .* bathtub

	# Collect inflows and outflows for each Stock
	dbathtub = faucet - leakage

	# State update
	dSdt[collect(1)] .= dbathtub
...
pars = (desired_amount = desired_amount,)

# Define initial condition in vector
xstart = [Base.Iterators.flatten([bathtub,])...]
xstart_names = [:bathtub]
...
intermediary_names = ["faucet", "leakage"]

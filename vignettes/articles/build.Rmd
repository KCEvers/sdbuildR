...
The flows make use of four other variables: `delta`, `gamma`, `alpha`, and `beta`. These are constants (i.e. static variables), and also need to be defined in the model. Below, we add these in a vectorized manner for efficiency. In addition, we document the meaning of these variables in the optional `doc` property.
...
As a first step in creating this stock-and-flow model, let's start by setting up our model. We change the name, and specify that we want to see Maya's burnout develop over the course of 9 months:
...
sfm = sfm %>%
  ...
  build("deterioration", "flow", 
        eqn = "work * sleep_reduction", from = "sleep") %>%
...
Maya's sleep is decreasing, but we haven't yet added the effect of her sleep deficit on how much she is working. The further Maya's sleep is from what she needs, the more she tries to reduce her work. We change the outflow from work to be determined by both her work and the ratio of the sleep she needs to the sleep she is getting.

```{r}
sfm = sfm %>%
  build("aim_for_normal", change_name = "sleep_limits_work", 
        eqn = "work * necessary_sleep / sleep") 
```

What's more, Maya's sleep deficit also makes itself worse, as she lies awake worrying about how little she is sleeping. We change the outflow of sleep to get larger when Maya is sleeping less than she needs. For good measure, we increase the amount of work that piles on.

```{r, fig.width=8, fig.height=4}
sfm = sfm %>%
  build("pile_on", eqn = 1.5) %>%
  build("deterioration", 
        eqn = "work * sleep_reduction * necessary_sleep / sleep") 
...
sfm = sfm %>% sim_specs(stop = 15) %>%
  build("deterioration", 
        eqn = "(work - normal_workday) * sleep_reduction * necessary_sleep / sleep") %>%

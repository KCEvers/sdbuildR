...
For guidance on installing and setting up the Julia environment, see `vignette(julia-setup)`. For all models, the simulation engine has to be changed to Julia:
...
```{r}
sfm <- model_units(sfm, "CO_2", change_name = "C02")
```
